...
| `ci-templates/email-proxy-app.gitlab-ci.yml` | email_proxy 本體 repo（VM + Compose） | test → build → 三映像（api/worker/smtp）推 registry → Trigger API 觸發本 infra repo 的 `ansible-deploy-app`（ADR-6） |
...
│
├── infra/email-proxy-infra          ← 本 monorepo（GitOps 唯一事實來源）
...
│   ├── ansible/、terraform/
│   │
│   │   include: project=infra/email-proxy-infra, file=gitlab/ci-templates/...
│   ├──────────────┬─────────────────────┬──────────────────────┐
│                  │                     │                      │
├── apps/web-backend            apps/web-frontend      apps/email-proxy
│   （backend 模板）             （frontend 模板）       （email-proxy-app 模板）
...
- **email_proxy（VM + Compose）**：CI 只 push 映像 + 觸發 infra 管線；
...
| email_proxy app repo | `INFRA_PROJECT_ID` / `INFRA_TRIGGER_TOKEN` | Masked, Protected | infra repo 的 Pipeline trigger token |
